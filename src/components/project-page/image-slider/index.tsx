    ...
    <div className="w-full">
      ...
      <div style={dotsContainerStyles}> 
        {slides.map((_, slideIndex) => (
          <div
            className={`${slideIndex === currentIndex ? 'text-primary' : 'text-base-content'} text-opacity-60 transform transition-transform duration-800 hover:scale-110 cursor-pointer`}
            key={slideIndex}
            style={{marginTop: '0.75rem'}}
            onClick={() => goToSlide(slideIndex)}
          >
            <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 24 24" width="1.75em" fill="currentColor"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2"></path></svg>
          </div>
        ))}
      </div>
    </div>
